...
- 研报与一致预期：东财研报、PDF 下载、同花顺一致预期、i问财研报语义搜索。
- 信号与题材：强势股、题材归因、北向资金、板块归属、个股资金流。
- 龙虎榜增强：个股龙虎榜席位、全市场龙虎榜、净买额排名、上榜原因。
- 资金面与筹码：融资融券、大宗交易、股东户数、分红送转、个股资金流 120 日。
- 新闻公告：个股新闻、全球资讯、巨潮公告。
- 基础资料：F10、财报三表、季报快照、市值、PE/PB、换手率等。

后续接入优先级：

1. 个股分析页：补研报、一致预期、新闻公告、F10 和财务摘要。
2. 风险标签：补限售解禁、融资融券、大宗交易、股东户数变化。
3. 推荐排名：补北向资金、个股 120 日资金流、题材归因、龙虎榜增强字段。
4. Qlib 特征工程：将上述新增字段沉淀成离线训练和回测特征。

当前已落地第一版“公司画像增强”盘后任务：先用问财 SkillHub 对涨停池股票补 `data/company-context/latest.json`，字段覆盖涨停原因、行业/概念、竞争对手、行业地位/市占率/龙头相关返回字段和题材唯一性说明。该任务的字段设计参考 `a-stock-data` 的 F10、研报、一致预期、行业对比和股东户数能力；后续如需更强的市占率/龙头证据，可把 `a-stock-data` 中 F10/研报/行业对比实现接为第二 provider。

接入注意：

- 该项目偏 Skill/参考实现形态，正式接入前需要把具体端点整理成项目内稳定 provider。
- 东财相关接口有风控风险，应保留限流、重试、缓存和失败 warning。
- 所有新增数据仍应遵守本系统原则：真实源失败时显式失败或 warning，不静默使用样例数据。
